...
- Instagram comment polling is a bounded catch-up fallback across paginated owned media. Webhook secrets are not treated as confirmed Meta dashboard registration; set `INSTAGRAM_COMMENT_WEBHOOK_CONFIRMED=true` only after a human confirms `comments` / `live_comments`.
