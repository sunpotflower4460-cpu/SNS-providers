...
const pwaWrangler = await readFile(new URL('../wrangler.toml', import.meta.url), 'utf8');
...
if (!xOwnedStore.includes('if (!fromCache)')
  ...
  || !store.includes('const fromCache = result.source === \'cache\'')
  || !store.includes('followingComplete && !fromCache')
  || !store.includes('isFollowing && followersComplete && followedAt && !fromCache')
  || !xOwnedStore.includes('function preservePostSnapshotFollowState(')
  || !xOwnedStore.includes('if (!candidate.followedAt)')
  || !xOwnedStore.includes('followedAt: prior.followedAt')
  ...
  || !backupControls.includes('const fingerprintAtConfirm = stateFingerprint(latestStateRef.current)')
  || !backupControls.includes('復元の確定中にこの端末のデータが変更されたため、上書きせず停止しました')
  || !syncControls.includes('let conflicted = false')
  || !syncControls.includes('復元中にこの端末のデータが変更されたため、上書きせず停止しました')
  || !syncControls.includes('latestStateRef.current = state')) {
...
if (!/^name = "sns-providers"$/m.test(pwaWrangler)
  ...
  || !pwaWrangler.includes('command = "npm run build"')
...
if (pkg.scripts.postinstall !== 'node scripts/workers-ci-build.mjs'
  ...
  // `secrets` is not available in a job-level `if:`; GitHub rejects the whole file.
  || /^\s{4}if:\s*\$\{\{\s*secrets\./m.test(workerDeploy)
  || !workerDeploy.includes('id: gate')
  || !workerDeploy.includes("steps.gate.outputs.enabled == 'true'")) {
...
for (const name of srcNames) {
  ...
  if (srcByLower.has(key)) {
    throw new Error(`Case-colliding src files break macOS APFS imports: ${srcByLower.get(key)} vs ${name}`);
  }
  srcByLower.set(key, name);
}
if (!srcNames.includes('missionInboxModel.ts') || !srcNames.includes('onboardingState.ts')) {
  throw new Error('Mission Inbox / onboarding modules lost Mac-safe filenames.');
}
