...
2. As repository administrator, open Settings → Pages → Build and deployment and select **GitHub Actions**. Then rerun **Deploy PWA to Pages**. Its previous `Resource not accessible by integration` failure occurred while `configure-pages` tried to create a site without administrator permission.
...
6. Repeat the core flow for seven days. Check keyboard and safe areas, VoiceOver labels, airplane mode and reconnection, midnight rollover, snooze, and app updates. Record each blocker with iPhone model, iOS version, URL, steps, and expected/actual result; fix any data loss or blocked core flow before calling the release ready.
