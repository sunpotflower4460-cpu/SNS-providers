...
2. As repository administrator, open Settings → Pages → Build and deployment and select **GitHub Actions**. Then run **Deploy PWA to Pages** on the review branch for pre-merge testing. Its previous `Resource not accessible by integration` failure occurred while `configure-pages` tried to create a site without administrator permission.
