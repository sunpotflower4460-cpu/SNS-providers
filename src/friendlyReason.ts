...
const rules: Array<[RegExp, string]> = [
  [/Failed to fetch|NetworkError|Load failed|Worker API.*(timeout|タイムアウト)/i, 'サーバーにつながりませんでした。通信状態を確認するか、設定 →「準備をはじめる」でサーバーの手順を確認してください。'],
  ...
  [/Personal control token is not configured/i, 'サーバーに個人管理キー（SYNC_TOKEN_SHA256）が登録されていません。設定 →「準備をはじめる」の「合言葉」の手順を行ってください。'],
  ...
  [/INSTAGRAM_ACCESS_TOKEN|Instagram token, professional user ID, and API version/i, 'Instagram連携がサーバーで未設定です。設定 →「Instagramとつなぐ」を行ってください。'],
